...
const WarningListCard: FC<Props> = ({ isVisible, header, subElements, preset }) => {
    ...
    const [subElementsDisplayed, setSubElementsDisplayed] = useState<
        string[] | undefined
    >(subElements);
    ...
    useEffect(() => {
        if (preset) {
            ...
            setSubElementsDisplayed([
                'The login must be at least 5 characters long, and be unique',
    ...
    if (isVisible) {
        return (
            <div className={stl.listCard}>
                ...
                <ul>
                    {subElementsDisplayed?.map((value, index) => (
